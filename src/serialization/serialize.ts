...
export const serialize = {
  bip32Path(path: number[] | string): Buffer {
    var pathArray =
      typeof path === "string" ? this.bip32PathAsArray(path) : path;
  ...
  bip32PathAsArray(path: string): number[] {
  ...
  arrayAndChunk<T>(
    ...
    for (let i = 0; i < Math.ceil(data.length / maxPacketSize); i++) {
      const chunks = [];
      for (
        let j = i * maxPacketSize;
        j < Math.min((i + 1) * maxPacketSize, data.length);
        j++
      ) {
